...
y_array = np.array(y)
probs_array = np.array(probs_cv)

for _ in range(n_boot):
    idx = rng.choice(len(y_array), len(y_array), replace=True)

    # Skip bootstrap samples containing only one class
    if len(np.unique(y_array[idx])) < 2:
        continue

    boot_aucs.append(
        roc_auc_score(y_array[idx], probs_array[idx])
    )
...
p_below = np.mean(boot_aucs >= 0.5)

print("Probability that true AUC >= 0.5:", p_below)
...
def fit_and_summarize(X, y, out_path=None, label="model", cv_splits=3):
    ...
    return results, clf, auc_cv, probs_cv
...
res_lang, clf_lang, auc_lang, probs_lang = fit_and_summarize(
...
res_nonlang, clf_nonlang, auc_nonlang, probs_nonlang = fit_and_summarize(
...
def bootstrap_auc_ci(y, probs, auc_value, label, n_boot=10000, seed=42):
    ...
    for _ in range(n_boot):
        ...
        boot_aucs.append(roc_auc_score(y_array[idx], probs_array[idx]))
...
bootstrap_auc_ci(y, probs_lang, auc_lang, "Language-only model")
bootstrap_auc_ci(y, probs_nonlang, auc_nonlang, "No-language model")
...
sum_df["Metric_label"] = sum_df["Metric"].replace({
    "Haiku_Wording": "Haiku Wording"
})
...
ax.set_yticks(y)
ax.set_yticklabels(sum_df["Metric_label"])
ax.set_xlim(1, 5)
...
metric_labels = [
    m.replace("Haiku_Wording", "Haiku Wording")
    for m in impression_metrics
]
...
#save it in PDF
plt.savefig(
    "ground_truth_vs_perceived_authorship.pdf",
    format="pdf",
    bbox_inches="tight"
)
...
"""### **Barchart for each model recognition accuracy**"""
...
model_acc = model_acc.sort_values("Correct")

fig, ax = plt.subplots(figsize=(7.5, 4.2))

# force white background
fig.patch.set_facecolor("#D6F2F4")
ax.set_facecolor("#D6F2F4")
...
for i, value in enumerate(model_acc["Correct"]):
    ax.text(
        value + 0.015,
        i,
        f"{value:.2f}",
...
ax.set_xlim(0, 0.75)
ax.set_xlabel("Recognition Accuracy", fontsize=10, color="#2F3A3A")
ax.set_ylabel("")

ax.set_title(
    "Human Recognition Accuracy Across Models",
    fontsize=15,
    fontfamily="serif",
    color="#168C91",
    pad=14
...
ax.grid(axis="x", linestyle="--", alpha=0.18, color="#6A8C8C")

ax.spines["top"].set_visible(False)
ax.spines["right"].set_visible(False)
ax.spines["left"].set_color("#7FAAAA")
ax.spines["bottom"].set_color("#7FAAAA")

ax.tick_params(axis="both", colors="#2F3A3A", labelsize=10)
ax.legend(frameon=False, fontsize=10)

plt.tight_layout()

plt.savefig(
    "human_recognition_accuracy_models_soft_white.pdf",
    format="pdf",
    bbox_inches="tight",
    facecolor="white"
)
